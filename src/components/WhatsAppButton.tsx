...
const WhatsAppButton = () => {
  ...
  useEffect(() => {
    const fetchContactInfo = async () => {
      try {
        ...
        if (response.ok) {
          const data: ContactInfoApiResponse = await response.json();
          setWhatsappNumber(data.msg_data.whatsapp_number);
        }
      } catch (error) {
  ...
  if (!whatsappNumber) {
    // console.log("WhatsApp number not loaded yet");
    return null;
  }
  ...
  let messageUrl = `https://wa.me/${whatsappNumber}`;

  if (isProjectPage) {
    const currentUrl =
      typeof window !== "undefined" ? window.location.href : "";
    messageUrl += `?text=I'm interested in this project: ${currentUrl}`;
  }
